...
// ------------------------------------------------------------------------------------------------
int user_slippage=2;
extern int grid_size=5;            // Grid_Size
extern int gs_progression=2;       // GS_Progression
/*
...
*/
extern int take_profit=8;          // Take_Profit
extern double profit_lock=0.34;      // Profit_Lock
...
extern int max_positions=1;         // Max_Position// if not used, set it to maximum => no restriction
extern int unbalance_control=1;     // unbalanced_Control
...
//int (btnWidth+btnGap)=btnWidth+btnGap;        // distance to next button
   ...
   if(!IsDemo())
     {
      ...
      MessageBox("Only on DEMO  account please!","CAUTION!",MB_OK);
      ...
      if(ObjectFind(0,"btnManualBuy")==-1)
        {
         ...
         DrawButton("btnManualSell","Sell",btnLeftAxis+(btnWidth+btnGap),btnTopAxis,btnWidth,btnHeight,false,colNeutral,clrBlack);
         DrawButton("btnCloseLastBuy","平前多",btnLeftAxis,btnTopAxis+btnNextTop,btnWidth,btnHeight,false,colNeutral,clrBlack);
         DrawButton("btnCloseLastSell","平前空",btnLeftAxis+(btnWidth+btnGap),btnTopAxis+btnNextTop,btnWidth,btnHeight,false,colNeutral,clrBlack);
         DrawButton("btnCloseAllBuys","平全多",btnLeftAxis,btnTopAxis+2*btnNextTop,btnWidth,btnHeight,false,colNeutral,clrBlack);
         DrawButton("btnCloseAllSells","平全空",btnLeftAxis+(btnWidth+btnGap),btnTopAxis+2*btnNextTop,btnWidth,btnHeight,false,colNeutral,clrBlack);
         DrawButton("btnShowComment","開/關 註解",5,btnTopAxis,btnWidth*1.5,btnHeight,false,colNeutral,colCodeYellow);

         DrawButton("btnstopNextCycle","Stop Next Cycle",btnLeftAxis+2*(btnWidth+btnGap),btnTopAxis,(int)(btnWidth*2.5),btnHeight,false,colNeutral,clrBlack);
         DrawButton("btnrestAndRealize","Rest & Realize",btnLeftAxis+2*(btnWidth+btnGap),btnTopAxis+btnNextTop,(int)(btnWidth*2.5),btnHeight,false,colNeutral,clrBlack);
         DrawButton("btnStopAll","Stop & Close",btnLeftAxis+2*(btnWidth+btnGap),btnTopAxis+2*btnNextTop,(int)(btnWidth*2.5),btnHeight,false,colNeutral,clrBlack);
        }
   ...
   if(show_forecast)
     {
      ...
      Write("panel_1_01",ChartSymbol(0),5,22,"Arial",12,instrumentCol);
      if(ter_spread>max_spread)
         Write("panel_1_02","Spread: "+DoubleToString(ter_spread/10,1),5,42,"Arial",8,colCodeRed);
      else
         Write("panel_1_02","Spread: "+DoubleToString(ter_spread/10,1),5,42,"Arial",8,panelCol);

      Write("panel_1_03",DoubleToString(CalculatePriceByTickDiff(relativeVolume,ter_tick_size*10),2)+" "+ter_currencySymbol+" / Pip",5,58,"Arial",8,panelCol);
      Write("panel_1_04","Balance: "+DoubleToString(AccountBalance(),2)+" "+ter_currencySymbol,5,74,"Arial",8,panelCol);
      Write("panel_1_05","Equity: "+DoubleToString(AccountEquity(),2)+" "+ter_currencySymbol,5,90,"Arial",8,panelCol);
      Write("panel_1_06","Free Margin: "+DoubleToString(AccountFreeMargin(),2)+" "+ter_currencySymbol,5,106,"Arial",8,panelCol);
      Write("panel_1_07","P/L Sym. "+DoubleToString(total_buy_profit+total_sell_profit,2)+" "+ter_currencySymbol,5,122,"Arial",8,instrumentCol);
      if(total_buy_profit<0)
         Write("panel_1_08","P/L Buy: "+DoubleToStr(total_buy_profit,2)+" "+ter_currencySymbol,5,144,"Arial",8,colInMinus);
      else
         Write("panel_1_08","P/L Buy: "+DoubleToStr(total_buy_profit,2)+" "+ter_currencySymbol,5,144,"Arial",8,colInPlus);
      if(total_sell_profit<0)
         Write("panel_1_09","P/L sell: "+DoubleToStr(total_sell_profit,2)+" "+ter_currencySymbol,5,160,"Arial",8,colInMinus);
      else
         Write("panel_1_09","P/L sell: "+DoubleToStr(total_sell_profit,2)+" "+ter_currencySymbol,5,160,"Arial",8,colInPlus);

      double accountPL=AccountProfit();
      if(accountPL<0)
         Write("panel_1_10","P/L Acc. "+DoubleToString(accountPL,2)+" "+ter_currencySymbol,5,176,"Arial",8,colInMinus);
      else
         Write("panel_1_10","P/L Acc. "+DoubleToString(accountPL,2)+" "+ter_currencySymbol,5,176,"Arial",8,colInPlus);
      ...
      Write("panel_1_11","Pips2Go B "+DoubleToString(pips2go_Buys,0)+" S "+DoubleToStr(pips2go_Sells,0),5,192,"Arial",8,panelCol);
     }
  }
//+------------------------------------------------------------------+
//| CheckAccountState
...
void ObjectButton(string ctlName,enObjectOperation Operation,string Text="",int X=-1,int Y=-1,int Width=-1,int Height=-1,bool Selected=false,color BgColor=clrNONE,color TextColor=clrNONE)
  {
   ...
   if((ObjectFind(ChartID(),ctlName)>-1))
   ...
   else if(Operation==LODraw)
     {
      ...
      ObjectSetInteger(ChartID(),ctlName,OBJPROP_COLOR,TextColor);
      ObjectSetInteger(ChartID(),ctlName,OBJPROP_BGCOLOR,BgColor);
      ObjectSetInteger(ChartID(),ctlName,OBJPROP_XDISTANCE,Dpi(X));
      ObjectSetInteger(ChartID(),ctlName,OBJPROP_YDISTANCE,Dpi(Y));
      ObjectSetInteger(ChartID(),ctlName,OBJPROP_XSIZE,Dpi(Width));
      ObjectSetInteger(ChartID(),ctlName,OBJPROP_YSIZE,Dpi(Height));
      ObjectSetInteger(ChartID(),ctlName,OBJPROP_STATE,Selected);
      ObjectSetString(ChartID(),ctlName,OBJPROP_FONT,"Arial");
      ObjectSetString(ChartID(),ctlName,OBJPROP_TEXT,Text);
      ObjectSetInteger(ChartID(),ctlName,OBJPROP_FONTSIZE,9);
